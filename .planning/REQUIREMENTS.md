...
- [ ] **FOUND-02**: Single Next.js 15 (App Router, RSC) project boots with Tailwind + shadcn pattern, deployed to Vercel
- [ ] **FOUND-03**: Subdomain middleware routes `proactivsports.com` → `app/(root)/`, `hk.proactivsports.com` → `app/(hk)/`, `sg.proactivsports.com` → `app/(sg)/`
