...
| 2026-04-23: Surface-default swap — cream → pure white on `--muted` + `--accent` | User preference during Phase 2 `/_design` visual verify: "focus on having a white background as standard instead of cream". Brand cream remains available via `bg-brand-cream` for deliberate accent use; surface hierarchy now comes from borders + shadow + photography rather than a warm tint. Navy-on-white contrast (14.55:1) is stronger than navy-on-cream (13.24:1) so no a11y regression. Sidebar nav `hover:bg-accent` becomes invisible by design — keyboard focus ring remains primary affordance; mouse-hover affordance to be revisited if Phase 3+ user testing surfaces it as a friction. | ✓ Good (amends Phase 2 D-01-color / UI-SPEC §1.4) |
