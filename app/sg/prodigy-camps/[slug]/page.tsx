// Phase 6 / Plan 06-05 — SG individual camp page with Event JSON-LD.
// Dynamic route: fetches camp by slug from Sanity, renders detail + structured data.
// priceCurrency defaults to SGD for Singapore market.
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${camp.title} | Prodigy Singapore Holiday Camps`,
    description: camp.description ?? undefined,
    alternates: { canonical: `https://sg.proactivsports.com/prodigy-camps/${slug}/` },
  };
}
...
export default async function SGCampPage({ params }: Props) {
  const { slug } = await params;
  const { data: camp } = await sanityFetch({
    query: campBySlugQuery,
    params: { slug },
  ...
  const jsonLd = {
    ...
    offers: camp.price
      ? {
          "@type": "Offer",
          price: camp.price,
          priceCurrency: camp.priceCurrency ?? "SGD",
      ...
      <Section size="lg" bg="default">
        <ContainerEditorial width="default">
          <div className="max-w-3xl">
            ...
            {camp.venue?.name && (
              <p className="text-body text-brand-green font-semibold mt-2">
                {camp.venue.name}
            ...
            {camp.price && (
              <p className="text-body text-muted-foreground mt-3">
                From {camp.priceCurrency ?? "SGD"} {camp.price}
              </p>
            )}
